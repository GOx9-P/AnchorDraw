...
| `semantic_anchor_sd15_hypersd_spatial_sigma_sweep_colab.ipynb` | Hyperparameter Sweep cho tham số $\sigma_D$ (`spatial_sigma_latent`) của phương pháp `WM-03` (Top-k + Adaptive Bilateral) kết hợp `Bootstrap=2` trên Layer `L04` (16x16) với SD1.5 + Hyper-SD 4-step (8 samples x 10 giá trị $\sigma_D = 80$ ảnh) kèm contact sheet ($2 \times 5$). |
